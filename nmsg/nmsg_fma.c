...
nmsg_fma_free(nmsg_fma fma, void *ptr) {
	...
	if (rec->num == 0) {
		...
		if (fma->debug_mode)
			fprintf(stderr, "freed %s block at %p@%#.zx, %u blocks total\n",
				fma->name, rec, fma->block_size, fma->num_allocated);
...
fma_new_block(nmsg_fma fma) {
	...
	if (fma->debug_mode)
		fprintf(stderr, "allocated %s block at %p@%#.zx, %u blocks total\n",
			fma->name, block->addr, fma->block_size, fma->num_allocated);
