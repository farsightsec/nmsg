...
cdef extern from "wdns.h":
    ...
    ctypedef struct wdns_dns_name_t:
        uint16_t            len
        uint8_t             *data

    ctypedef struct wdns_dns_rdata_t:
        uint16_t            len
        uint8_t             data[0]

    ctypedef struct wdns_dns_qrr_t:
        uint16_t            rrtype
        uint16_t            rrclass
        wdns_dns_name_t     name

    ctypedef struct wdns_dns_query_t:
        uint16_t            id
        uint16_t            flags
        wdns_dns_qrr_t      question

    ctypedef struct wdns_dns_rr_t:
        uint32_t            rrttl
        uint16_t            rrtype
        uint16_t            rrclass
        wdns_dns_name_t     name
    ...
    ctypedef struct wdns_dns_rrset_t:
        uint32_t            rrttl
        uint16_t            rrtype
        uint16_t            rrclass
        uint16_t            n_rdatas
    ...
    ctypedef struct wdns_dns_rrset_array_t:
        uint16_t            n_rrsets
        wdns_dns_rrset_t    **rrsets

    ctypedef struct wdns_dns_message_t:
        uint16_t            id
        uint16_t            flags
        wdns_dns_qrr_t      question
    ...
    wdns_msg_status wdns_parse_message(uint8_t *op, uint8_t *eop, wdns_dns_message_t *m)

    wdns_msg_status wdns_parse_message_rr(uint8_t *p, uint8_t *eop, uint8_t *data, size_t *rrsz, wdns_dns_rr_t *rr)

    wdns_msg_status wdns_parse_question_record(uint8_t *q, uint8_t *eoq, wdns_dns_qrr_t *question)
